...
// are not present upstream: the notable-only pokemon scan and the wipe-a-species
// cache operation. Kept in their own file (and registration function) so
// rebasing onto upstream never conflicts inside upstream's route functions.
