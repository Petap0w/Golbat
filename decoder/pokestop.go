...
func GetPokestopRecord(ctx context.Context, db db.DbDetails, fortId string) (*Pokestop, error) {
	...
	err := db.GeneralDb.GetContext(ctx, &pokestop,
		`SELECT pokestop.id, lat, lon, name, url, enabled, lure_expire_timestamp, last_modified_timestamp,
			pokestop.updated, quest_seed, quest_type, quest_timestamp, quest_target, quest_conditions,
			...
			quest_expiry, alternative_quest_seed, alternative_quest_expiry, description, showcase_pokemon_id, showcase_pokemon_form_id,
	...
	if oldPokestop == nil {
		res, err := db.GeneralDb.NamedExecContext(ctx, `
			INSERT INTO pokestop (
				id, lat, lon, name, url, enabled, lure_expire_timestamp, last_modified_timestamp, quest_seed, quest_type,
				quest_timestamp, quest_target, quest_conditions, quest_rewards, quest_template, quest_title,
				alternative_quest_seed, alternative_quest_type, alternative_quest_timestamp, alternative_quest_target,
		...
		res, err := db.GeneralDb.NamedExecContext(ctx, `
			UPDATE pokestop SET
				...
				alternative_quest_seed = :alternative_quest_seed, 
