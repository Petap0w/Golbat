...
Returned (JSON error object) when `pokedex_id` is not an integer, is negative,
or exceeds `32767`.
...
1. **Cache only — not the database.** This evicts entries from Golbat's
   ...
   already persisted to MariaDB are left untouched. After a wipe, the species
   disappears from `/api/pokemon/id/{encounter_id}` immediately; it disappears
   from `/api/pokemon/scan*`, `/api/pokemon/notable`, and `/api/pokemon/search`
   results as the eviction pipeline drains — index cleanup is asynchronous
   (single dispatcher goroutine plus batched tree workers) and typically lands
   within milliseconds to a few seconds of the response, bounded by the ~1 s
   scan-snapshot refresh.
...
7. **Very large wipes.** Each removed entry produces one eviction event on a
   bounded dispatcher queue shared with normal TTL expiries; if that queue
   overflows, the excess events are dropped and their lookup-index entries leak
   until restart (visible in the dropped-evictions metric). Tens of thousands
   of removals in one call are fine; avoid firing wipes of several very common
